...
function MemberVerifyForm() {
  ...
  async function onSubmit(e: React.FormEvent) {
    ...
    setError("");
    setLoading(true);
    const res = await fetch("/api/auth/member/verify", {
      method: "POST",
    ...
    >
      ...
      {devCode && (
        <div className="mb-6 rounded-2xl border border-brand-200 bg-brand-50 px-4 py-3 text-sm text-brand-700">
          <strong>Development mode:</strong> no real SMS/email is sent yet. Your code is{" "}
      ...
      {!devCode && !delivered && (
        <div className="mb-6 rounded-2xl border border-danger-border bg-danger-bg px-4 py-3 text-sm text-danger-text">
          We couldn't send your verification code — SMS/email delivery isn't configured yet. Contact
          the church office to sign in.
        </div>
      )}
